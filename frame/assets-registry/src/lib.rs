...
pub mod pallet {
	...
	pub type ForeignAssetLocation<T: Config> = StorageMap<
		_,
		Blake2_128Concat,
		T::LocalAssetId,
		T::Location,
	...
	pub type FromForeignAssetLocation<T: Config> = StorageMap<
		_,
		Blake2_128Concat,
		T::Location,
		T::LocalAssetId,
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn approve_assets_mapping_candidate(
			...
			Self::approve_candidate(who, local_asset_id, foreign_asset_id.clone(), location, decimals)?;
			...
			match current_candidate_status {
				...
				Some(CandidateStatus::LocalAdminApproved) =>
					if Some(who) == foreign_admin {
						Self::promote_candidate(local_asset_id, foreign_asset_id, location, decimals)?;
					},
